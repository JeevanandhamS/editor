...
public class SpannedXhtmlGenerator {
    ...
    public String toXhtml(Spanned src) {
        ...
        if (spans.length == 0) {
            ...
            for (AlignmentSpan span : spans) {
                ...
                if (src.length() > spanEnd + 1
                        && (src.charAt(spanStart) == '\n' || src.charAt(spanEnd + 1) == '\n')) {
                    spanEnd = spanEnd + 1;
                }
    ...
    private void chunkToXhtml(SpannableStringBuilder result, Spanned src,
        ...
        if(!checkForBulletSpan(result, src, align)
                && !checkForQuoteSpan(result, src)) {
            result.append(blockToXhtml(src, align));
        }
    }

    private boolean checkForBulletSpan(SpannableStringBuilder result, Spanned src, Layout.Alignment align) {
        ...
        for (BulletSpan span : spans) {
            ...
            if (!inBulletRun) {
                ...
                if (spanStart > 0) {
                    ...
                    result.append(blockToXhtml(subSequence(src, subsequenceStart, subsequenceEnd), null));
                }
            ...
            result.append("<li>");
            result.append(blockToXhtml(subSequence(src, spanStart, spanEnd), null));
            result.append("</li>");
        ...
        if (lastSpanEnd < src.length()) {
            result.append(blockToXhtml(subSequence(src, lastSpanEnd, src.length()), null));
        }

        return true;
    }

    private boolean checkForQuoteSpan(SpannableStringBuilder result, Spanned src) {
        QuoteSpan[] spans = src.getSpans(0, src.length(), QuoteSpan.class);
        if(spans.length == 0) {
            return false;
        }

        int lastSpanEnd = -1;
        boolean inQuoteRun = false;
